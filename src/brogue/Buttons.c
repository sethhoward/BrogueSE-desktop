...
// iOS port (Brogue SE): report the active menu's window rect to the host for the iPhone menu
...
// Teardown is host-driven when play resumes (uiMode → InNormalPlay flips gameplayControlsActive);
// nested menus (e.g. inventory → item detail → inventory) just overwrite, no clear between.
...
short buttonInputLoop(brogueButton *buttons,
    ...
    // iOS port (Brogue SE): iPhone menu magnify. Expand by a 1-cell "trim" HORIZONTALLY only, so a
